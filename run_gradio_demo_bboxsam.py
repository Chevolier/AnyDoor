...
image_list.sort()
image_dict_list = [{"image": image} for image in image_list]
...
def create_mask(image, bbox):
    ...
    mask = np.zeros_like(image[:, :, 0], dtype=np.uint8)  # Create a mask with the same shape as the image

    # Set the bounding box region in the mask to 1
